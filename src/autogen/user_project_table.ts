...
export class UserProjectTableHandle {
  ...
  /**
   * Access to the `id` unique index on the table `UserProject`,
   * which allows point queries on the field of the same name
   * via the [`UserProjectIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.userProject.id().find(...)`.
   *
   * Get a handle on the `id` unique index on the table `UserProject`.
   */
  id = {
